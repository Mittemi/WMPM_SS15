 ...
 */
public class RefillStation implements Processor {
    @Override
    public void process(Exchange exchange) throws Exception {
        Claim claim =exchange.getIn().getBody(Claim.class);
        System.out.println("RefillStation fixed "+claim.getDescription() + " for car with ID="+exchange.getIn().getHeader("carId)") );
    }
}
